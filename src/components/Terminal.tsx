...
export const Terminal = () => {
  ...
  const generateEquation = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('generate-equation', {
        body: {}
      });
      ...
      <div className="max-w-4xl mx-auto space-y-4">
          ...
          <div className="p-6 space-y-4 max-h-[600px] overflow-y-auto">
            {messages.map((msg, idx) => (
              <div key={idx} className="space-y-2">
                ...
                {msg.type === 'equation' && (
                  <div className="p-4 bg-muted/50 rounded border border-primary/30">
                ...
                {msg.type === 'proof' && msg.proof && (
                  <div className="p-4 bg-card rounded border border-accent/30 space-y-2">
                ...
                {msg.type === 'verification' && (
                  <div className={`p-4 rounded border flex items-center gap-3 ${
                    ...
                    <p className={`font-mono text-lg font-bold ${
                      msg.verified ? 'text-secondary' : 'text-destructive'
                    }`}>
                      {msg.content}
                    </p>
                  </div>
        ...
        <form onSubmit={handleSubmit} className="flex gap-2">
          <Input
            ...
            placeholder='Type "generate" or enter equation...'
            className="flex-1 bg-input border-primary/30 text-foreground font-mono focus:ring-primary"
            disabled={isLoading}
          />
          <Button 
            type="submit" 
            disabled={isLoading || !input.trim()}
            className="bg-primary hover:bg-primary/90 text-primary-foreground font-mono terminal-glow"
          >
